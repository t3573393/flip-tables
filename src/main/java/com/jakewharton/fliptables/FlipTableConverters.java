...
/**
 * Helper methods for creating {@link FlipTable tables} from various different
 * data sets.
 */
public final class FlipTableConverters {
	...
	public static String fromObjects(String[] headers, Object[][] data) {
		...
		String[][] dataArray = (String[][]) stringData.toArray(new String[stringData.size()][]);
		return FlipTable.of(headers, dataArray);
	}

	/**
	 * Create a table from a group of objects. Public accessor methods on the
	 * class type with no arguments will be used as the columns.
	 */
	public static <T> String fromIterable(Iterable<T> rows, Class<T> rowType) {
		...
		for (T row : rows) {
			...
			for (int column = 0; column < columnCount; column++) {
				try {
					rowData[column] = String.valueOf(((Method) methods.get(column)).invoke(row));
				} catch (Exception e) {
		...
		String[] headerArray = (String[]) headers.toArray(new String[headers.size()]);
